...
public class TicTacToeManager : MonoBehaviour, IPunTurnManagerCallbacks
{
    ...
    void OnPlayerMove(object move)
    {
        ...
        string gridName = moveMessage[0].Trim();   // 操作されたマス目のオブジェクト名
        string mark = moveMessage[1].Trim();       // 〇 or ×
    ...
    bool Judge()
    {
        for (int i = 0; i < 3; i++)
        {
            if (_grids[i * 3].Mark == _grids[i * 3 + 1].Mark && _grids[i * 3 + 1].Mark == _grids[i * 3 + 2].Mark &&
                _grids[i * 3].Fixed && _grids[i * 3 + 1].Fixed && _grids[i * 3 + 2].Fixed)
            {
                Endgame(_grids[0].Mark);
                return true;
            }   // 横に揃っている時

            if (_grids[i].Mark == _grids[i + 3].Mark && _grids[i + 3].Mark == _grids[i + 6].Mark &&
                _grids[i].Fixed && _grids[i + 3].Fixed && _grids[i + 6].Fixed)
            {
                Endgame(_grids[0].Mark);
                return true;
            }   // 縦に揃っている時
        ...
        if (_grids[2].Mark == _grids[4].Mark && _grids[4].Mark == _grids[6].Mark &&
            _grids[2].Fixed && _grids[4].Fixed && _grids[6].Fixed)
        {
            Endgame(_grids[2].Mark);
            return true;
        }   // 左上⇔右下に渡って斜めに揃っている時

        if (_grids[0].Mark == _grids[4].Mark && _grids[4].Mark == _grids[8].Mark &&
            _grids[0].Fixed && _grids[4].Fixed && _grids[8].Fixed)
        {
            Endgame(_grids[0].Mark);
            return true;
        }   // 右上⇔左下に渡って斜めに揃っている時
